...
public:
    ...
    //Z1.3
    Vojnik(Vojnik&& obj) {
        _zdravlje = move(obj._zdravlje);
        _izdrzljivost = move(obj._izdrzljivost);
        _oruzja = move(obj._oruzja);
        _statusZiv = obj.StatusZiv;
        _moral = obj._moral;
        obj._moral = nullptr;
    }
    ...
    int GetZdravlje() const {
        assert(_zdravlje != nullptr && "Greska! '_zdravlje' je 'nullptr' [Vojnik::GetZdravlje]");
        return *_zdravlje;
    }
    int GetIzdrzljivost() const {
                assert(_izdrzljivost != nullptr && "Greska! '_izdrzljivost' je 'nullptr' [Vojnik::GetIzdrzljivost]");
        return *_izdrzljivost;
    }
    Moral GetMoral() const {
        assert(_moral != nullptr && "Greska! '_moral' je 'nullptr' [Vojnik::GetMoral]");
        return *_moral;
    }
...
bool operator == (const Vojnik& v1, const Vojnik& v2) {
        return v1.Zdravlje == v2.Zdravlje
        && v1.Izdrzljivost == v2.Izdrzljivost
        && v1.Moral_ == v2.Moral_
        && v1.Oruzja == v2.Oruzja
        && v1.StatusZiv == v2.StatusZiv;
}
bool operator != (const Vojnik& v1, const Vojnik& v2) {
    return !(v1 == v2);
}
