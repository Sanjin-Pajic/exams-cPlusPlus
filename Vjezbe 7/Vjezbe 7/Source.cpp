...
bool CheckIfIsti(const char* tekst1, const char* tekst2) {
    if (tekst1 == nullptr || tekst2 == nullptr) return false;
...
bool CheckIfIsti(const vector<const char*>& v1, const vector<const char*>& v2) {
    if (v1.size() != v2.size()) return false;
    for (size_t i = 0; i < v1.size(); i++)
        if (!CheckIfIsti(v1[i], v2[i])) return false;
    return true;
}
...
public:
    ...
    static const char* GetRandomId() {
        return IntToStr(rand() % 90000 + 10000);
    }
    ...
    //Z1.2
    Uredjaj(const char* naziv, int godinaProizvodnje, float cijena, int garancija): _godinaProizvodnje(godinaProizvodnje) {
        _id = GetRandomId();
    ...
    Uredjaj(const Uredjaj& obj): _godinaProizvodnje(obj._godinaProizvodnje), _ocjene(obj._ocjene) {
    ...
    //Z1.4
    Uredjaj(Uredjaj&& obj):_naziv(move(obj._naziv)),
        _godinaProizvodnje(move(obj._godinaProizvodnje)),
        _cijena(move(obj._cijena)),
...
bool operator == (const Uredjaj& u1, const Uredjaj& u2) {
        if (!CheckIfIsti(u1.Naziv, u2.Naziv) || u1.GodinaProizvodnje != u2.GodinaProizvodnje
        || u1.Cijena != u2.Cijena || u1.Garancija != u2.Garancija || u1.Ocjene != u2.Ocjene)
        return false;
    return true;
}
bool operator != (const Uredjaj& u1, const Uredjaj& u2) {
...
public:
    ...
    Mobitel(const Mobitel& mobitel) : Uredjaj(mobitel) {
        strcpy_s(_operativniSistem, 50, mobitel.OperativniSistem);
        _velicinaEkrana = make_unique<float>(mobitel.VelicinaEkrana);
        _memorija = new int(mobitel.Memorija);
    }
    //Z2.4
    Mobitel(Mobitel&& obj):Uredjaj(move(obj)) {
        strcpy_s(_operativniSistem, 50, obj._operativniSistem);
        strcpy_s(obj._operativniSistem, 50, "");
    ...
    float GetVelicinaEkrana()const { 
        assert(_velicinaEkrana != nullptr && "GetVelicina GRESKA!");
        return *_velicinaEkrana; 
    }
    int GetMemorija()const { 
        assert(_memorija != nullptr && "GetMemory GRESKA");
        return *_memorija; }
    ...
    //Z2.7
    ~Mobitel() {
        _velicinaEkrana.reset(); // moze biti izostavljeno
        delete _memorija;
        _memorija = nullptr;
    }
};
...
bool operator == (const Mobitel& m1, const Mobitel& m2) {
    if ((Uredjaj&)m1 != (Uredjaj&)m2 || !CheckIfIsti(m1.OperativniSistem, m2.OperativniSistem)
        || m1.VelicinaEkrana != m2.VelicinaEkrana || m1.Memorija != m2.Memorija)
        return false;
    return true;
}
bool operator != (const Mobitel& m1, const Mobitel& m2) {
...
public:
    ...
    Televizor(const Televizor& obj) : Uredjaj(obj) {
        _velicinaEkrana = make_unique<float>(obj.VelicinaEkrana);
        _tipEkrana = UniqueAlokator(obj.TipEkrana);
        _rezolucija = obj.Rezolucija;
        _prikljucci = obj.Prikljucci;
    }
    //Z3.4
    Televizor(Televizor&& obj) :Uredjaj(move(obj)),
        _velicinaEkrana(move(obj._velicinaEkrana)),
    ...
    float GetVelicinaEkrana()const {
        assert(_velicinaEkrana != nullptr && "GetVelicinaEkrana GRESKA!");
        return *_velicinaEkrana;
    }
    const char* GetTipEkrana()const {
        return _tipEkrana.get();
    }
    ...
    Televizor& operator = (const Televizor& obj) {
        ...
        Rezolucija = obj.Rezolucija;
        Prikljucci = obj.Prikljucci;
        return *this;
    }
    ...
    ~Televizor() {
        _velicinaEkrana.reset(); // moze biti izostavljeno
        _tipEkrana.reset(); // moze biti izostavljeno
    }
};

bool operator == (const Televizor& t1, const Televizor& t2) {
        if ((Uredjaj&)t1 != (Uredjaj&)t2 ||
        t1.VelicinaEkrana != t2.VelicinaEkrana ||
        !CheckIfIsti(t1.TipEkrana, t2.TipEkrana) ||
        t1.Rezolucija != t2.Rezolucija ||
        !CheckIfIsti(t1.Prikljucci, t2.Prikljucci))
        return false;
    return true;
}
bool operator != (const Televizor& t1, const Televizor& t2) {
...
public:
    ...
    int GetBrzinaPrintanja() const {
        assert(_brzinaPrintanja != nullptr && "GRESKA : GetBrzinaPrintanja je nullptr");
...
bool operator == (const Printer& p1, const Printer& p2) {
    if ((Uredjaj&)p1 != (Uredjaj&)p2 || p1.BrzinaPrintanja != p2.BrzinaPrintanja)
        return false;
    return true;
}
bool operator != (const Printer& p1, const Printer& p2) {
...
public:
    ...
    WebShop(const WebShop& obj) {
        ...
        for (int i = 0; i < _trenutnoUredjaja; i++) {
            ...
            auto tel = dynamic_cast<Televizor*>(obj._uredjaji[i]);
            if (tel != nullptr) {
                _uredjaji[i] = new Televizor(*tel);
                continue;
            }
            auto prin = dynamic_cast<Printer*>(obj._uredjaji[i]);
            if (prin != nullptr) {
                _uredjaji[i] = new Printer(*prin);
                continue;
            }
            auto uredjaj = dynamic_cast<Uredjaj*>(obj._uredjaji[i]);
            if (uredjaj != nullptr)
...
bool operator == (const WebShop& shop1, const WebShop& shop2) {
    if (!CheckIfIsti(shop1.Naziv, shop2.Naziv) || !CheckIfIsti(shop1.EmailAdresa, shop2.EmailAdresa))
        return false;
    if (shop1.TrenutnoUredjaja != shop2.TrenutnoUredjaja)
        return false;
    for (int i = 0; i < shop1.TrenutnoUredjaja; i++)
        if (*shop1.GetElementAt(i) != *shop2.GetElementAt(i))
            return false;
    return true;
}
