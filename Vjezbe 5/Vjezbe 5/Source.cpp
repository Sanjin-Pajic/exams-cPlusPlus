...
ostream& operator << <>(ostream& COUT, const DinamickaKolekcija<T>& kolekcija) {
    for (size_t i = 0; i < kolekcija._trenutno; i++)
        COUT << kolekcija._elementi[i] << endl;
    return COUT;
}
...
Sahista* VratiNajmladjegSahistu(DinamickaKolekcija<Sahista>& sahisti) {
    Sahista* najmladji = &sahisti[0]; // postavljanje vrijednosti na adresu prvog elementa kolekcije
...
public:
    ...
    Par(Par<T1, T2>&& obj)
        :_lijevi(move(obj._lijevi)),
        _desni(move(obj._desni)) 
    {
    }
