...
/*bool ValidirajBrojTelefona(string broj) {
    return regex_match(broj, regex("[+][0-9]{2,3}\\s?[0]?[0-9]{2}\\s?[0-9]{2,3}?\\s?\\-?[0-9]{2,3}"));
}*/
...
public:
    ...
    friend bool operator==(Kolekcija k1, Kolekcija k2) {
        if (k1.getTrenutno() != k2.getTrenutno()) return false;
        for (size_t i = 0; i < k1.getTrenutno(); i++)
        {
            if (k1.getElement1(i) != k2.ge)
        }
    }

    Kolekcija operator() (int pocetak, int kraj) {
        if (pocetak < 0 || kraj > _trenutno) throw exception("van opsega");
...
public:
    ...
    Predmet& operator=(const Predmet& obj) {
        if (this == &obj) return *this;
        delete[] _naziv; _naziv = GetNizKaraktera(obj._naziv);
        _ocjene = obj._ocjene;
        return *this;
    }

    void AddOcjena(Datum d1, int ocjena) {
        
        _ocjene->AddElement(new Datum(d1), ocjena);
    }
    ...
    // ukoliko predmet nema niti jednu ocjenu prosjecna treba biti jednaka 0
        ...
        {
            COUT << "Ocjena: " << obj._ocjene->getElement2(i) << "\t | Datum ocjene: " << *obj._ocjene->getElement1(i) << endl;
        }
        return COUT;
    }
    ...
    friend bool ProvjeriJesuLiIsti(Predmet p1, Predmet p2);
};
...
public:
    ...
    Uspjeh(const Uspjeh& obj): _polozeniPredmeti(obj._polozeniPredmeti) {
        _razred = obj._razred;
    }
...
bool ProvjeriJesuLiIsti(Predmet p1, Predmet p2) {
    if (strcmp(p1._naziv, p2._naziv) == 0 && p1._ocjene == p2._ocjene) return true;
    return false;
}
