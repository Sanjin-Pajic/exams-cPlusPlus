 ...
 */
	return regex_search(lozinka, regex("(?=.{7,})(?=.*[a-z]{1,})(?=.*[A-Z]{1,})(?=.*\\W{1,})"));
}
...
public:
	...
	void AddElement(T1 el1, T2 el2) {
		if (*_trenutno == max) throw exception("Max dosegnut");
...
public:
   ...
   evidentirane (bez obzira sto je stariji od 3 dana)*/
		...
		{
			if (razlikaDana(datumOcjene, _ocjeneRjesenja->getElement2(trenutno)) < 3 || datumOcjene.toDays() > _ocjeneRjesenja->getElement2(i).toDays() || trenutno != 0) return false;
		}
	...
	friend ostream& operator<<(ostream& COUT, Pitanje& obj) {
		...
		{
			COUT << "Ocjena: " << obj._ocjeneRjesenja->getElement1(i) << "\t Datum ocjene: " << obj._ocjeneRjesenja->getElement2(i) << endl;
		}
		return COUT;
	}
	~Pitanje() {
