...
//   jedno mjesto unazad
	...
	void AddElement(T1 el1, T2 el2) {
		if (*_trenutno == max) throw exception("Max scope niza dosegnut");
	...
	*/
		...
		if (*_trenutno == max) return; //obrambeni mehanizam, jer nije naglaseno da bacamo exception :)
	...
	T1& operator[] (T2 vrijednost) {
		...
		for (int i = 0; i < *_trenutno; i++)
		{
			if (*_elementi2[i] == vrijednost) return *_elementi1[i];
		}
	}
...
public:
	...
	Tehnika(const Tehnika& obj): _ocjene(obj._ocjene){
	...
	bool AddOcjena(int ocjena, Datum datumOcjene) {
		...
		*/
		...
		if (razlikaDana(datumOcjene, *_ocjene.getElement2(trenutno - 1)) < 3) return false;
...
public:
	...
	Polaganje(const Polaganje& obj): _polozeneTehnike(obj._polozeneTehnike) {
		_pojas = obj._pojas;
	}
