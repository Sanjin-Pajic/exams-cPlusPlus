...
public:
    ...
    Vrijeme(const Vrijeme& obj) {
        assert(!CheckIfNull(obj) && "Vrijeme::Copy ctor - GRESKA");
    ...
    Vrijeme(Vrijeme&& obj) {
        ...
        obj._sati = obj._minute = obj._sekunde = nullptr;
    }
    //Z1.6
    ...
    static Vrijeme GetRandomVrijeme() {
        Vrijeme vrijeme;
        vrijeme.SetSati(rand() % 24);
        vrijeme.SetMinute(rand() % 60);
        vrijeme.SetSekunde(rand() % 60);
        return vrijeme;
    }
    //Z1.10
    bool CheckIfValidnoVrijeme() {
        if (*_sati > 23 || *_sati < 0) return false;
        if (*_minute > 59 || *_minute < 0) return false;
        if (*_sekunde > 59 || *_sekunde < 0) return false;
        return true;
    }
    //Z1.11
    ~Vrijeme() {
...
public:
    ...
    static bool CheckIfNull(const DatumVrijeme& obj) {
        if (obj._dan == nullptr || obj._mjesec == nullptr || obj._godina == nullptr) return true;
        return false;
    }
    //Z2.2
    DatumVrijeme() {}
    ...
    DatumVrijeme& operator =(const DatumVrijeme& obj) {
        ...
        assert(!CheckIfNull(obj) && "DV= greska");
...
ostream& operator << (ostream& COUT, const DatumVrijeme& obj) {
    COUT << obj.Dan << "." << obj.Mjesec << "." << obj.Godina << " " << (Vrijeme&)obj;
    return COUT;
}
//Z2.13
...
//Z2.14
bool operator >(const DatumVrijeme& d1, const DatumVrijeme& d2) {
    int total1 = d1.Godina * 365 + d1.Mjesec * 30 + d1.Dan;
    int total2 = d2.Godina * 365 + d2.Mjesec * 30 + d2.Dan;
    if (total1 > total2)
        return true;
    else if (total1 == total2)
        return (Vrijeme&)d1 > (Vrijeme&)d2;
    return false;
}
bool operator >= (const DatumVrijeme& d1, const DatumVrijeme& d2) {
    return (d1 > d2) || (d1 == d2);
...
public:
    ...
    T& operator [] (int index) const {
        ...
        assert(_elementi[index] != nullptr && "Array<T>::operator[] - GRESKA!");
...
public:
    ...
    DatumVrijeme GetDatumRodjenja() const { return *_datumRodjenja; }
...
bool operator == (const Osoba& o1, const Osoba& o2) {
    ...
    if (strcmp(o1.Prezime, o2.Prezime) != 0)
        return false;
    if (o1.DatumRodjenja != o2.DatumRodjenja)
        return false;
...
public:
    ...
    Pjevac& operator = (const Pjevac& obj) {
        ...
        _zanr = Alokator(obj._zanr);
        _aktivan.reset(); 
        _aktivan = make_unique<char>(*obj._aktivan);
...
bool operator == (const Pjevac& p1, const Pjevac& p2) {
    ...
    if (strcmp(p1.Zanr, p2.Zanr) != 0)
        return false;
    if (p1.Instrumenti != p2.Instrumenti)
        return false;
...
public:
    ...
    //Z6.3
    Pjesma(const Pjesma& obj): _trajanje(obj._trajanje) {
        _naziv = Alokator(obj._naziv);
        _tekst = Alokator(obj._naziv);
    ...
    //Z6.4
    Pjesma(Pjesma&& obj):_trajanje(move(obj._trajanje)) {
        _naziv = obj._naziv;
        obj._naziv = nullptr;

        _tekst = obj._tekst;
        obj._tekst = nullptr;
        _tekstopisac = move(obj._tekstopisac); // ERROR ?
        _trenutnoZanrova = obj._trenutnoZanrova;
        for (size_t i = 0; i < _trenutnoZanrova; i++)
        {
    ...
    const char* GetZanr(int index) const {
        ...
        index = Min<int>(index, _trenutnoZanrova - 1);
...
public:
    ...
    Album(Album&& obj):_pjesme(move(obj._pjesme)), _izdavackeKuce(move(obj._izdavackeKuce)) {
        _naziv = obj._naziv; obj._naziv = nullptr;
        _pjevac = obj._pjevac; obj._pjevac = nullptr;
    }
    //Z7.5
    Album& operator =(const Album& obj) {
        ...
        delete[] _naziv;
        _naziv = Alokator(obj._naziv);
        assert(obj._pjevac != nullptr && "Album::copy assignment operator - GRESKA!");
        if (_pjevac == nullptr)
            _pjevac = new Pjevac;
        *_pjevac = *obj._pjevac;
        _pjesme = obj._pjesme;
    ...
    ~Album() {
        ...
        delete _pjevac; _pjevac = nullptr;
    }
};
//Z7.11
...
bool operator == (const Album& a1, const Album& a2) {
    if (strcmp(a1.Naziv, a2.Naziv) != 0)
        return false;
    if (a1.Pjevac_ != a2.Pjevac_)
        return false;
    if (a1.Pjesme != a2.Pjesme)
        return false;
    if (a1.IzdavackeKuce != a2.IzdavackeKuce)
        return false;
    return true;
}
bool operator !=(const Album& a1, const Album& a2) {
