...
public:
    ...
    //Z1.8
    bool CheckIfValid() {
        if (*_godina > INT_MAX || *_godina < INT_MIN) return false;
        if (*_mjesec > 12 || *_mjesec < 1) return false;
        if (*_dan > 31 || *_dan < 1) return false;
...
public:
    ...
    void operator += (T noviElement) {
        if (_elementi == nullptr) _elementi = new T[_maxElemenata];
        if (_trenutno == _maxElemenata) ProsiriVektor(10);
    ...
    bool CheckIfSadrzi(T element) {
        for (size_t i = 0; i < _trenutno; i++)
        {
            if (_elementi[i] == element) return true;
        }
        return false;
    }
...
public:
    ...
    Fudbaler(const Fudbaler& obj) {
        ...
        for (size_t i = 0; i < _trenutnoNagrada; i++)
        {
    ...
    bool operator()(const char* nagrada) {
        if (_trenutnoNagrada == 100) return false;
    ...
    void SetDatumRodjenja(Datum datumRodjenja) {
        if (_datumRodjenja == nullptr) _datumRodjenja = make_unique<Datum>();
...
public:
    ...
    FudbalskiTim(FudbalskiTim&& obj) :
        _datumOsnivanja(move(obj._datumOsnivanja)),
        _fudbaleri(move(obj._fudbaleri)),
