...
public:
    ...
    Clan& operator = (const Clan& obj) {
        ...
        strcpy_s(_korisnickoIme, 30, obj._korisnickoIme);
        strcpy_s(_lozinka, 20, obj._lozinka);
        if (_datumRegistracije == nullptr) _datumRegistracije = new Datum;
        _datumRegistracije = obj._datumRegistracije;
        if (_spol == nullptr)
            _spol = new bool;
        *_spol = *obj._spol;
...
public:
    ...
    Post(const char* korisnickoIme, Datum datumO, const char* sadrzaj)
        : _datumObjavljivanja(datumO.GetDan(), datumO.GetMjesec(), datumO.GetGodina()) {
...
ostream& operator <<(ostream& COUT, const Post& p) {
    ...
    COUT << "Datum: " << p._datumObjavljivanja << endl;
...
public:
    ...
    bool operator +=(Post& p) {
        if (_trenutnoPostova == maxBrojPostova) return false;
...
public:
    ...
    Forum(Forum&& obj) {
        ...
        for (size_t i = 0; i < _trenutnoSekcija; i++)
            _sekcije[i] = obj._sekcije[i];  //ovde se koristi operator = od klase Sekcija
   ...
   //     Kopirati sadrzaj iz starog niza u novi niz
    ...
    void operator += (const Clan clan) {
        if (_trenutnoClanova == _maxClanova) SetMaxClanova(_maxClanova + 10);
